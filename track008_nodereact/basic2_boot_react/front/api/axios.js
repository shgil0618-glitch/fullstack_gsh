...
const api = axios.create({
  ...
  // refresh token이 httpOnly 쿠키에 저장이 되어 있으면 자동 포함 필요
  headers: {
...
// 요청 인터셉터 : 요청 보내기전에 Acess Token을 헤더에 추가
api.interceptors.request.use(
  (config) => {
    if (typeof window !== "undefined") {    // -> CRS 환경에서만 LocalStoraga 접근
      const accessToken = localStorage.getItem("accessToken");     // -> 저장된 Acess Token 가져오기
      if (accessToken) {
        config.headers.Authorization = `Bearer ${accessToken}`;     // -> Authoriazation 헤더에 추가
      }
    }
    return config;  
  },
