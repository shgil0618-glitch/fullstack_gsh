...
class BackApplicationTests3_Service {
   ...
   void testAppUserRepository() { 
	   ...
	   // 프로필이미지 변경
	   MultipartFile file = new MockMultipartFile("file", "test.png", "image/png", "dumy".getBytes());
   ...
   void testPostService() {
	   ...
	   // 게시글 수정
	   PostRequestDto updatedReq = new PostRequestDto("수정된 게시글","#newTag");
	   PostResponseDto updated = postService.updatePost(user1Dto.getId(), post.getId(), updatedReq, null);
   ...
   void testRetweetService() { 
       ...
       assertThat(retweet.getRetweetCount()).isEqualTo(1);	// 리트윗수가 1이야
       // 중복리트윗 -> 예외처리
