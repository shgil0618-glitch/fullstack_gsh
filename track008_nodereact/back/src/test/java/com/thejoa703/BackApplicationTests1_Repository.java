...
/**
 * Repository CRUD 통합테스트 순서 : AppUser →
 */
...
class BackApplicationTests1_Repository {
	...
	// -------------------------------------------------------
	@Test
	@DisplayName("■ AppUserRepository-CRUD ") // 진행중인 테스트 이름
	...
	// -------------------------------------------------------
	@Test
	@DisplayName("■ ImageRepoisory-CRUD ") // 진행중인 테스트 이름 / 어떤 유저가 어떤 글을 작성했다. ★
	void testImageRepoisory() {
		// 이미지 생성(save)
	...
	// -------------------------------------------------------
	@Test
	@DisplayName("■ HashtagRepoitory-CRUD ") // 진행중인 테스트 이름 / 어떤 유저가 어떤 글을 작성했다. ★
	void testHashtagRepoitory() {
	...
	// -------------------------------------------------------
	// PostRepoisory - HashTag
	// -------------------------------------------------------
	@Test
	@DisplayName("■ PostRepoisory-CRUD ") // 진행중인 테스트 이름 / 어떤 유저가 어떤 글을 작성했다. ★
	void testPostRepoisory() {
		// 해쉬태그 연결 후 검색
		Hashtag tag = new Hashtag();
		tag.setName("haha");
		hashtagRepository.save(tag);
		...
		for (int i = 0; i < 5; i++) {
			Post extra = new Post();
			extra.setContent("게시글" + 1);
			extra.setUser(user1);
			extra.setDeleted(false);
			postRepository.save(extra);
		}
		// 전체글 페이징
		List<Post> pagedPosts = postRepository.findPostWithPaging(1, 3);
...
//---------------------------------------------------------------------
	...
	void testFollowRepository() {
		...
		assertThat(followings.get(0).getFolloweeId().getId()).isEqualTo(user2.getId());
		...
		assertThat(followers.get(0).getFollowerId().getId()).isEqualTo(user1.getId());
...
/*
 * 사용자 관리자 CREATE ◎회원가입 ◎회원가입 READ 로그인, 이메일중복, 닉네임중복 UPDATE ◎닉네임수정, ◎이미지수정
 * DELETE ◎회원탈퇴
 * 
 */
