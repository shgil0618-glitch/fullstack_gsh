...
import com.thejoa703.config.PasswordConfig;
...
//    }
	...
	public UserResponseDto signup(UserRequestDto request, MultipartFile profileImage) {
		...
		user.setUfile(profileImage != null && !profileImage.isEmpty() ? fileStorageService.upload(profileImage) // 업로드된
																												// 이미지
																												// 저장
				: DEFAULT_PROFILE_IMAGE); // 기존프로필 이미지 사용
		//////////////////////////////////////// DB 저장후 DTO 반환
	...
	public UserResponseDto login(LoginRequest request) {
		// db 정보 : 이메일 + provider로 사용자 조회, 없으면 예외
	...
	public UserResponseDto updateNickname(Long userId, String newNickname) {
		...
		// 닉네임 조회 후
		AppUser user = appUserRepository.findById(userId).orElseThrow(() -> new IllegalArgumentException("사용자 없음"));	//##1. 조회
		user.setNickname(newNickname); // ##2. 닉네임 세팅
		return UserResponseDto.fromEntity(appUserRepository.save(user)); // ##3. save 변경 저장 후 dto 변환
	}
	...
	public UserResponseDto updateProfileImage(Long userId, MultipartFile profileImage) {
		...
		return UserResponseDto.fromEntity(appUserRepository.save(user)); // 저장 후 dto 반환
	}

	// delete : id 삭제
	public void deleteById(Long userId) { appUserRepository.deleteById(userId); }
	// 전체 사용자 수
	public long countUser() { return appUserRepository.count(); }
	// 이메일 중복 여부
	...
	public String findRoleByUserId(Long userId) {
		return appUserRepository.findById(userId)		// userId로 사용자 조회 Optioal<AppUser> 반환
								.map(AppUser::getRole)	// 조회된 사용자가 있으면 getRole(0 권환 반
								.orElse("ROLE_USER");	// 
	}
}
