 ...
 * passport/local.js
 * ---------------------- 
 * 사용자가 로그인 시 입력한 email과 passport를 DB와 비교하여 인증합니다.
...
module.exports = ()=>{
    passport.use(
        new LocalStrategy(
            {usernameField: 'email',   // 로그인시 사용할 필드명 res.body.email
             passwordField: 'password',// 로그인시 사용할 필드명 res.body.password
            }, // 설정
            async (email, password, done)=>{
                try{
                    const user = await findUserByEmail(email);//DB에서 이메일로 사용자 조회
                    if(!user){return done(null, false, {message: '존재하지 않는 이메일 입니다.'}); }
                    //                   (에러X,인증실패,실패정보)
