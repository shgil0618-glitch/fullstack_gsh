 ...
 *   post:
 *     summary: 회원가입(테스트용)
 *     description: 새로운 사용자를 등록합니다. (인증 미적용)
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *   post:
 *     summary: 로그인(테스트용)
 *     description: Passport LocalStrategy 이메일/비밀번호 인증 후 세션저장
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
...
router.post('/login', async(req,res,next)=>{
    passport.authenticate('local',(err,user,info)=>{ 
        ...
        req.login(user , (loginErr) => { // passport가 제공하는 함수, 인증된 사용자객체를 세션에 저장하는 과정
            // 내부적으로 serializeUser 호출되면 세션에 저장, 이후 deserializeUser 싱행되서 req.user사용자 객체 복원
