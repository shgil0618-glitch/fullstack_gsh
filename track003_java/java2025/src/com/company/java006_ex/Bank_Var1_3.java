		...
		for (;;) {
			...
			case 0:
				...
				if (level >= 1 && level <= 5 || level == 9) {
					if (add == false && level != 1) {
						System.out.println("로그인 혹은 회원가입 부탁드립니다. (1번)");
						level = 0;
						continue;
					} else {
						break;
					...
					System.out.println("올바른 메뉴를 눌러주세요");
					level = 0;
					continue;
				}
			...
			case 1:
				...
				if (past_id.equals(ids)) {
					System.out.println("이전과 같은 아이디는 사용할수 없습니다.");
					continue;
				}
				System.out.print("pw : ");
			...
			case 3:
				...
				for (;;) {
					...
					if (come > 0) {
						...
						System.out.println("0또는 음수의 값은 허용하지 않습니다.");
						continue;
					}
				}
				break;
			...
			case 4:
				...
				for (;;) {
					...
					if (out <= left) {
						...
						System.out.println("출금 금액은 잔액보다 클 수 업습니다.");
					}
				}
				break;
